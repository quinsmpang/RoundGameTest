	...
	for i=0,#equipTb / 3 + 0.99 do
		...
		if i == 0 then
			...
			local EquipBtn = EquipItemBtn.new(equip, nil, function (  )
				...
				self:equipDetail(equip, true)
			end,function ( event )
				--local spawn = cca.spawn({cca.moveTo(0.2, 600, 0), cca.fadeOut(0.3)})
				self.detailBg:runAction(cca.seq({cca.moveTo(0.2, 400, 0), cca.callFunc(function ( node )
...
function HeroEquipDetailLayer:equipDetail( equip, istemp )
	if istemp then
		self.detailNode = nil
		self.detailBg = Funcs.newMaskedSprite("heros/package_detail_bg_alpha_mask", "heros/package_detail_bg.jpg")
		:pos(0, 0)
	...
	if istemp then
		self.detailBg:runAction(cca.moveTo(0.2, 140, 0))
	end
