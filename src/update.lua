...
function UpdateScene:ctor()
    ...
    :addTo(self)
        ...
        -- 进度条
    local sp = display.newSprite("LoginPanel/com_lemongame_fastlogin_or_bind.png")
    self.progressTimer = cc.ProgressTimer:create(sp)
        :pos(display.cx, display.cy)
        :addTo(self)
    self.progressTimer:setType(1)
...
function UpdateScene:onResponse(event, index, dumpResponse)
    ...
    local total = 0
    if event.dltotal then
        total = event.dltotal
    end
    self.progressTimer:setPercentage(total / 320000 * 100)
    --dump(event, "----------")
